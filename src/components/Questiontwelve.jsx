...
export default function QuestionTwelve() {
  ...
  const handleSubmit = async (e) => {
    ...
    const fields = {
      fields: {
        questiontwelve: student.fields.questiontwelve + 1,
      },
    };
  ...
  return (
    <>
      <h1>Who do you think is the biggest soccer fan?</h1>
      <Video />
      <div>
        ...
        {classmates.map((image) => (
          ...
          <Link to='/Winners' onClick={handleSubmit} style={{ display: 'inline-block', width: 170, height: 170, borderRadius: 400 / 2, }}><img style={{ width: 170, height: 170, borderRadius: 400 / 2 }}
            src=
              {image.fields?.image}
            title={image.id}
          /></Link>
